...
8. **라이선스 준수** — 개인 개발이지만 회사 자산 점검에 활용하고 공개 대시보드/Issue에 재게시하므로:
   - ① 룰 재게시 시 **출처·author·라이선스 고지 보존** (SigmaHQ DRL 1.1은 author 표기가 의무)
   - ② **Exploit-DB PoC·KrCERT 공지 원문은 재게시 금지, 링크만** — AI 프롬프트 컨텍스트로의 내부 사용은 무방
   - ③ **AI 무료 티어(Gemini/Groq)에 회사 내부 자산 정보 입력 금지** — 공개 CVE 데이터만 전송 (Gemini 무료 티어는 입출력이 구글 제품 개선에 사용될 수 있음)

### 데이터 소스 라이선스 검토 결과 (2026-07-10, 실제 약관 확인)

검토 축: **내부 사용**(회사 자산 점검, AI 프롬프트 컨텍스트) / **재배포**(공개 Issue·대시보드에 재게시)

#### ✅ 완전 안전 — 내부 사용·상업적 사용·재배포 모두 허용

| 소스 | 라이선스 | 조건 |
|---|---|---|
| CVE (cvelistV5) | CVE ToU | 복제·재배포·상업적 사용 허용, CVE 저작권 고지 유지 |
| NVD | 미 정부 저작물 (퍼블릭 도메인) | 제한 없음 |
| CISA KEV | CC0 | 제한 없음 |
| CISA vulnrichment | CC0 1.0 | 제한 없음 — P5 최우선 소스로 확정 |
| EPSS (FIRST) | MIT | 상업적 사용 포함 무료, attribution 권장 |
| nuclei-templates | MIT | 제한 없음 |
| SigmaHQ 룰 | DRL 1.1 | 상업적 사용·재배포 명시 허용. 재게시 시 **author 표기 필수** (룰 YAML의 author 필드 보존으로 충족) |
| ET Open | MIT (OISF 공식 suricata-update 인덱스 기준) | 일부 레거시 이식 룰(SID 1–3464)은 GPLv2 — 재게시 시 헤더 고지 보존 |
| Metasploit metadata | BSD-3-Clause | 제한 없음 |
| Groq free tier | 서비스 약관 | 상업적 사용 금지 조항 없음. 금지: 재판매, rate limit 우회 |
| Supabase / GitHub Actions·Pages | free tier 약관 | 공개 프로젝트 무료 사용 정상 범위 |

#### ⚠️ 조건부 — 배제 불필요, 사용 방식만 제약

| 소스 | 이슈 | 대응 |
|---|---|---|
| Yara-Rules/rules | GPL-2.0 | 사용·재배포 가능. 룰 원문 게시 시 출처 링크+라이선스 표기 유지 |
| Exploit-DB | 개별 exploit 코드 저작권은 각 제출자, 명시 라이선스 없는 경우 다수 | 내부 사용(AI 컨텍스트)은 안전. **PoC 원문 재게시 금지, 링크만** |
| Gemini free tier | 라이선스 아닌 프라이버시: 무료 티어 입출력을 구글이 제품 개선에 사용 가능(인간 리뷰 포함) | 공개 CVE 데이터만 전송 (불변 원칙 8-③) |
| KrCERT/KISA | KOGL(공공누리) 표시 없는 공지는 재게시 제약 가능 | 원문 재게시 금지, **링크+요약만** |

**결론: 배제해야 할 소스는 없음.** 모든 소스가 "개인 개발 + 회사 자산 점검 활용 + 공개 재게시" 시나리오에서 사용 가능. 불변 원칙 8의 3가지 준수 사항만 지키면 된다.
...
2. **Exploit-DB: Code Search API → CSV 매핑 + 직접 조회**
   ...
   - **라이선스 준수**: PoC 코드는 **AI 프롬프트 컨텍스트 전용**. Issue/대시보드에는 원문 대신 Exploit-DB **링크만** 게시 (불변 원칙 8-②)
...
3. **룰 신뢰 등급(trust tier)**: 룰마다 `trust: official-verified | ai-validated | ai-draft`를 `rules_snapshot`에 저장 (기존 필드 안에 문자열 하나 추가라 DB 부담 없음) → Issue·대시보드에 등급 배지
   - **라이선스 준수**: official 룰 게시 시 trust 배지와 함께 **출처·author·라이선스(DRL 1.1/GPL-2.0/MIT) 표기** 보존 (불변 원칙 8-①)
...
| 소스 | 방식 | 제공 신호 |
|---|---|---|
| **CISA vulnrichment (ADP)** | 이미 다운로드하는 cvelistV5 레코드의 ADP 컨테이너 파싱 (추가 네트워크 비용 0, CC0) | SSVC Exploitation, CVSS/CWE 보강 — **최우선 ROI** |
| **ExploitDB** | P2에서 구축한 `files_exploits.csv` 매핑 재활용 | `has_public_exploit` 1급 신호 + 룰 프롬프트 컨텍스트 |
| **Metasploit** | `rapid7/metasploit-framework`의 `db/modules_metadata_base.json` 캐시 (BSD-3) | `has_metasploit_module` — "무기화됨" 신호 |
| **KrCERT/KISA** (선택) | RSS/HTML 스크래핑 — 유지보수 취약, 후순위. **원문 재게시 금지, 링크+요약만** (불변 원칙 8-②) | 국내 보안공지 "KR 공지" 배지 |
