...
지금은 남은 시간과 **남은 RPD/TPD** 로 예산을 잡는다(`main._translation_budget`).
...
| RPD 와 **TPD 를 함께** 센다 | TPD 는 아예 추적하지 않았다. 요청 수는 남았는데 토큰만 소진되면 429 를 받고 나서야 알게 된다 |
...
일일 한도는 AI Studio 에서만 확인할 수 있고 계정·모델마다 다르므로 코드가 아니라
**저장소 Variables** 로 넣는다 — `ARGUS_RPD_GEMINI`, `ARGUS_TPD_GEMINI`,
`ARGUS_RPD_GEMINI_FB`, `ARGUS_TPD_GEMINI_FB`, `ARGUS_RPD_ANALYSIS`,
`ARGUS_TPD_ANALYSIS`, `ARGUS_RPD_ANALYSIS_FB`, `ARGUS_TPD_ANALYSIS_FB`.
비워 두면 기본값(RPD 14,400 / TPD 미설정)을 쓴다.
...
| **offset 페이징의 정렬 키는 `id`처럼 안 바뀌는 값이어야 한다** (`export_cves`·`get_translation_backfill_candidates`·`tracked_states` 전부) | `updated_at DESC` + offset 은 fast-lane 이 쓰는 동안 행이 페이지 경계를 넘나들어 건너뛰고 중복된다. export 건수가 회차마다 달라지고, 번역·백필 순회는 영영 수렴하지 않는다 |
