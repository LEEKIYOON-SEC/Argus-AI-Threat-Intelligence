...
| **KPI 타일과 필터 칩은 같은 술어(`cveHasSignal`)를 쓴다** | 타일 숫자와 클릭했을 때 나오는 행 수가 어긋난다. 예전엔 타일이 티어 기준, 칩이 신호 기준이라 갈라졌다 |
| 타일 강조색 = 같은 신호의 배지 색 (`--kpi-accent` 하나로 레일·선택배경·테두리 공유) | 목록에서 KEV 배지를 본 눈이 위쪽 타일을 다른 것으로 읽는다 |
| 크게 쓰는 단독 수치에 `tabular-nums` 를 쓰지 않는다 | 모든 숫자가 `0` 너비를 받아 30px 에서 헐거워 보인다. 세로로 맞출 표의 열에만 쓴다 |

**KPI 5개**: 추적 중 CVE(전체·클릭하면 신호 필터 해제) · 🚨 악용 중 · 🧨 무기화 ·
🔬 PoC 공개 · 🧠 AI 발견. 전부 필터 칩과 1:1이고 클릭하면 그 필터가 걸린다.
