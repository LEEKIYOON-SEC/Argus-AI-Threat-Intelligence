...
def generate_korean_summary(cve_data: Dict) -> Tuple[str, str]:
    """
    Gemini를 사용한 한국어 번역 (v3.0 - 429 재시도 대응)
    
    v3.0 변경사항:
    - rate_limit_manager.check_and_wait("gemini") 사전 대기
    - 429 수신 시 Retry-After 파싱 후 대기 → 재시도 (최대 2회)
    - 재시도 실패 시 원본 사용
    """
    prompt = f"""
...
"""
    ...
    for attempt in range(max_retries + 1):
        try:
            # ✅ rate_limit_manager로 호출 간격 조절 (Lock 내부에서 처리)
            ...
            # ✅ 429 전용 처리: Retry-After 파싱 후 대기 → 재시도
