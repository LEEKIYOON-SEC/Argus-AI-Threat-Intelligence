...
<sub>CVE-2023-7028 (GitLab · CVSS 10.0 · EPSS 94.65% · CISA KEV 등재) — 위협 신호, 공격 벡터,
영향 버전 범위, 공개 탐지 룰(출처·라이선스 포함)을 한 화면에서 보고 AI 상세 분석 리포트로 이동합니다.</sub>
...
대응 우선순위 → 위험도 배지 → 위협 신호 → 한국어 개요 → 영향 자산 → 패치 목표 버전 →
AI 심층 분석(근본 원인 · 공격 벡터 · MITRE ATT&CK · 비즈니스 영향) → 대응 방안 → 공개 탐지 룰 순입니다.</sub>
