...
**추적 중 12,019건** · 상세 리포트 **2,216건** 발행 · 매시간 자동 실행 · 서버 없이 무료 티어에서

<sub>수치는 2026-08-16 기준 — 라이브 대시보드에서 현재 값을 볼 수 있습니다.</sub>
...
Linux 커널 CVE는 전부 `Linux / Linux`로 표기돼 제품 구분이 없고 목록의 41%를 차지합니다.
커널 필터가 **포함 · 제외 · 커널만**을 전체 건수와 함께 제공합니다(`커널 제외 (4,879건 숨김)`).
...
| **화면** | 실제 배포 데이터(12,005행)를 브라우저에 띄워 렌더·필터·모달·내보내기와 콘솔 오류 |
