...
| | 기능 | 설명 |
|:--:|---|---|
| 🎯 | **자산 기준 위험도 티어링** | 등록 자산(`assets.json`)은 저위험까지 추적, 미등록(전체 감시)은 **고위험만** 수신해 알림 노이즈 차단 |
...
```
                    ┌─────────────── GitHub Actions (정기 실행, 무료) ───────────────┐
                    │                                                                │
  공개 데이터 소스  │   ①수집        ②위험도 분류        ③AI 분석        ④전달        │
 ───────────────►   │  CVE·KEV·EPSS   자산 매칭·티어링   Groq/Gemini    Issue·Slack   │
  CVE / CISA / NVD  │  SSVC·PoC 신호  Critical/High/Low  한국어 분석    대시보드       │
                    │       │                                            │           │
                    └───────┼────────────────────────────────────────────┼───────────┘
                            ▼                                            ▼
                   Supabase (메타데이터 최소 저장)          GitHub Pages 대시보드 / Slack
```
...
1. 저장소를 Fork 후, `assets.json`에 감시할 벤더/제품을 등록합니다. (`*`/`*` = 전체 감시)
   ```json
   { "active_rules": [ { "vendor": "microsoft", "product": "exchange_server" } ] }
   ```
