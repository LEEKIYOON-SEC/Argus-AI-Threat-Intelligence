...
<details>
<summary><b>커널 변종 168종을 접고, 그 이름을 별칭으로 남긴 이유</b></summary>

Ubuntu는 커널을 배포 대상별로 따로 빌드해 소스 패키지를 쪼갭니다 — `linux-aws`,
`linux-azure-5.13`, `linux-oem-6.8`, `linux-riscv-5.11` … 한 CVE에 118개까지 붙습니다.
실측으로 이 변종들이 인덱스 용량의 **99%**를 먹고 있었습니다(커널 4,882건이 22.0MB,
나머지 1,446건은 다 합쳐 0.2MB).

용량보다 나쁜 건 리포트였습니다. 패치 버전 표에는 행 상한이 있고 정렬은 패키지명 알파벳순이라,
143행짜리 커널 CVE는 `Ubuntu Pro 14.04` 같은 남의 환경이 앞자리를 차지하고 정작 필요한 행이
잘려 나갔습니다 — 패치 표가 실리는 리포트의 **56%**가 그랬습니다.

그래서 기본 `linux`가 있는 CVE에서는 변종을 접습니다(20.8MB → **1.1MB**). 다만 접기만
하면 `linux-aws`를 쓰는 호스트의 커널 CVE가 **통째로 안 잡힙니다**(재현: AWS 4,277건 ·
Azure 4,884건 전량 누락). 사라진 이름 168종을 인덱스에 별칭으로 함께 실어(약 3KB) 대조하는
쪽이 `linux`로 되돌려 찾게 했습니다.

접는 규칙을 `linux-*`로 좁힌 것도 실측 때문입니다. "기본 이름이 있으면 접미 변종을 버린다"는
넓은 규칙을 돌려봤더니 `firefox-esr` · `xz-utils` · `grub2-signed`처럼 **실제로 다른 소스
패키지 58종**이 함께 사라졌습니다. 이름 대조가 그 이름으로 이뤄지므로 조용한 누락이 됩니다.

최종 확인은 인덱스의 고유 패키지명 **1,134종 × (CVE, 이름) 조합 596,432개 전수 대조**로
했습니다 — 접기 전에 잡히던 것 중 접은 뒤 못 잡는 조합 **0개**.
...
(`커널 제외 (4,879건 숨김)`). 예전에는 목록 안에서 접었다 펴는 방식이었는데, 페이지를 자른
뒤에 접다 보니 머리글의 건수가 그 50행 안의 수였고 커널만 들어찬 페이지에서는 데이터 행이
0개인 빈 화면이 나왔습니다(241페이지 중 79페이지). 같은 일을 필터가 정확하게 하므로 걷어냈습니다.
...
만들 때 맞다고 생각한 것들이 돌려보니 틀린 경우가 많았습니다. 아래는 **추측이 아니라 실제
운영 데이터로 확인해서** 고친 것들입니다. 숫자는 전부 배포 중인 데이터(추적 CVE 12,005건 ·
패키지 인덱스 6,328건)로 잰 값입니다.

| 무엇이 잘못돼 있었나 | 어떻게 드러났나 | 결과 |
| :--- | :--- | :--- |
| **백필 결과가 대시보드에 영영 안 나왔다** | 백필이 남긴 '전량 export' 표시를 다음 매시간 실행이 워터마크를 저장하며 통째로 덮어썼다. 몇 번을 돌려도 화면이 그대로였던 이유 | 상태를 덮어쓰지 않고 병합 → 백필이 다음 export에 반영 |
| **번역 백필이 대상을 한 건도 못 봤다** | 후보를 '최근 갱신순 200건'에서 골랐는데, 영문으로 굳은 행은 정의상 갱신이 멈춘 과거 행이라 항상 그 창 밖. 영문 잔존 111건 중 창 안에 든 건 **0건** | 회전 스캔으로 전체를 2.5일에 한 바퀴 |
| **SBOM이 취약한 호스트를 안전하다고 답했다** | 내 배포판에 답이 없으면 아무 배포판이나 집어 비교. Ubuntu 24.04 호스트에서 **441건**이 남의 배포판 숫자로 '수정판 추정' | 배포판·릴리스가 정확히 맞을 때만 '수정판', 아니면 '버전 미확인' → **0건** |
| **패치 버전 표가 절반 넘게 잘렸다** | 커널 변종이 인덱스 용량의 99%. 143행 중 12행만 실려 정작 필요한 배포판이 잘림 — 패치 표가 실리는 리포트의 **56%** | 변종을 접고(20.8MB → **1.1MB**) 상한을 올려 **0.1%**(6/6,328) |
| **목록에 빈 페이지가 나왔다** | 커널 접기가 페이지를 자른 뒤에 동작해, 커널만 든 페이지는 데이터 행 0개. **241페이지 중 79페이지** | 접기를 없애고 커널 필터로 일원화 → 모든 페이지 50행 |
| **일별 추이가 8개월치를 30칸에 그렸다** | `최근 30개 날짜`를 `최근 30일`로 착각. 공개일 기준으로 바꾸자 값이 드문드문해져 드러남 | 오늘부터 30일 창을 고정하고 빈 날은 0으로 |
| **DB egress가 무료 한도를 넘었다** | 매시간 90일치 전 행을 통째로 읽어 회당 ~15MB × 24회 ≈ 월 10GB+ (한도 5GB) | 바뀐 행만 읽어 병합 → **월 1GB 미만** |
| **저장소가 계속 커졌다** | 데이터 파일을 매시간 커밋. 최근 30일 커밋 323건이 전부 그것 (`.git` 100MB) | Pages 아티팩트 배포 → **시간당 커밋 0건** |
| **AI가 날마다 다른 모델로 분석했다** | 비상 티어의 일일 토큰 한도가 자주 차서, 같은 성격의 CVE인데 리포트 기준이 흔들림 | 공급자를 하나로 좁히고 역할마다 2단 + 정형 폴백 |
| **화면에 아무 일도 안 하는 것들이 있었다** | `룰 없음` 필터는 추적 CVE의 **98.9%**가 해당돼 변별력 0. 이슈의 조치 체크리스트는 실제로 쓰이지 않음 | 둘 다 제거. 공개 JSON의 빈 룰 껍데기 2,081건도 함께 |
| **공개 대시보드에 자산이 드러날 뻔했다** | 자산을 등록하는 순간 어떤 CVE가 우리 것인지 정적 파일에 그대로 노출 | 노출 경로를 UI·CSV·JSON에서 전부 제거, 판정은 DB에만 |

고친 것보다 **어떻게 찾았는지**가 중요했습니다. 대부분은 코드를 읽어서가 아니라 배포 중인
데이터를 직접 받아 세어보다 나왔습니다. "잘 도는 것 같다"와 "12,005행을 세어봤다" 사이의
차이가 이 표입니다.
...
| 무료 한도 | 어떻게 지키나 |
| :--- | :--- |
| **Supabase** egress 5GB/월 | 매시간 전량이 아니라 바뀐 행만 읽어 병합. 보존 정책이 120일 지난 대용량 JSONB를 비우고 180일 지난 행을 지운다 |
| **GitHub Actions** 2,000분/월 | 실행당 소프트 데드라인 38분. 초과 전에 스스로 마무리하고 워터마크를 저장해, 다음 실행이 그 지점부터 이어받는다 |
| **GitHub Pages** | 데이터 파일을 커밋하지 않고 아티팩트로 배포 — 저장소가 커지지 않는다 |
| **Google AI Studio** | 역할별 RPM·TPM·RPD를 실행 간에 이어붙여 추적. 한도에 닿으면 다음 모델, 그것도 안 되면 정형 폴백 |
