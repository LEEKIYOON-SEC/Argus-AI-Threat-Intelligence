...
그래서 유효한 벤더가 하나도 없으면 NVD CPE를 조회해 채웁니다. 예전에는 이 조회를
자산 매칭에 필요할 때만 했는데(전체 감시면 어차피 다 매칭되니 생략), 그 부작용으로
표시·필터용 벤더까지 같이 잃어 추적 CVE의 1.6%가 벤더 없이 남아 있었습니다.
NVD에도 CPE가 없으면 방법이 없습니다 — 표본상 절반 정도가 그렇습니다.
...
안전하다고 보고하게 됩니다(실측으로 Ubuntu 24.04 호스트에서 441건이 그런 오답이었습니다).
반대로 `취약 추정`은 릴리스가 달라도 알립니다 — 이쪽 오답은 확인 한 번으로 끝나니까요.
...
이슈는 읽는 리포트입니다. 작업 티켓으로 쓰라고 체크리스트를 넣어봤지만 실제로는 쓰이지 않아 걷어냈고,
공개 룰이 없을 때 "없습니다"를 설명하던 안내문도 뺐습니다 — 리포트의 98.9%가 그 경우라
본문만 길어질 뿐이었습니다. 판단에 필요한 근거만 남기고, 대응 관리는 이미 쓰던 도구에서 합니다.
발행 시각과 내용이 그대로 남으므로 사후 점검·감사 때 참조할 자료로는 계속 쓰입니다.
