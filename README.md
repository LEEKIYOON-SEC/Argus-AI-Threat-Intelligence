...
**매시간 자동 실행** · 위협 정보 10곳을 함께 확인 · 한국어 리포트 자동 작성 · 판단 시각과 근거 보존
...
이걸 하려면 우리가 뭘 쓰고 있는지부터 알아야 합니다. 그래서 **내 Linux·Windows 서버에서**
`syft`를 돌려 무엇이 설치돼 있는지 목록(SBOM)을 먼저 만들었습니다.
...
| :--- | :--- |
| 무엇을 쓰고 있나 | `syft`로 뜬 SBOM에서 벤더·제품 이름을 골라 `assets.json`에 등록해 두고, 신규 CVE를 그 목록과 맞춰봅니다 |
| 무엇이 급한가 | 실제 악용 중인지(KEV·SSVC), 악용될 확률(EPSS), 공격 도구 공개 여부를 묶어 3단으로 나눕니다 |
| 어디까지 올려야 하나 | 배포판·릴리스별 **패치 목표 버전**을 리포트와 대시보드에 함께 적습니다 |
| 언제 알았고 무엇을 했나 | 판단 시각과 근거가 GitHub Issue에 남습니다. 지우거나 고치지 않습니다 |
| 어제 안전했던 게 오늘도 안전한가 | 낮게 분류한 것도 30일간 매시간 다시 봅니다. KEV에 오르면 즉시 최상위로 올립니다 |
...
| | 기능 | 설명 |
|:--:|---|---|
| 🎯 | **등록 자산 이름으로 먼저 걸러내기** | `assets.json`에 적어둔 벤더·제품에 걸린 것만 통과. 등록 자산은 저위험까지 추적, 미등록(전체 감시)은 고위험만 수신 |
| 🚨 | **실제 악용 신호 우선** | CISA KEV · EPSS · Metasploit · ExploitDB · CISA SSVC를 종합해 "공격 도구가 이미 공개됐거나 실제 악용 중"인 건을 맨 위로 |
...
| 📈 | **위험도 오르면 다시 알림** | 낮게 분류했던 CVE가 KEV 등재·EPSS 급등으로 고위험이 되면 그 자리에서 다시 알립니다 |
| 🔁 | **빠뜨리지 않는 수집** | 어디까지 처리했는지 기록해 두고, 실행이 끊기면 다음 회차가 그 지점부터 이어받습니다 |
| 📦 | **패치 목표 버전** | OSV.dev 자료로 CVE와 패키지를 이어 "어느 버전까지 올리면 되는지"를 배포판·릴리스별로 표시 |
| 🗂️ | **조치 근거 보존** | 발행 시각과 판단에 쓴 신호, 데이터 출처가 GitHub Issue에 그대로 남습니다. 사후 점검·감사에서 요구되는 "언제 알았고 무엇을 근거로 판단했나"가 그대로 기록이 됩니다 |
| 📅 | **주간 요약 리포트** | 직전 주 탐지 결과(신규 · KEV 등재 · 공격 도구 공개 · 영향 제품 TOP)를 GitHub Issue로 자동 발행 |
...
드러나기 때문입니다. 이름이 걸렸는지 여부는 DB에 남아 위험도 분류와 알림에 계속 쓰입니다.
어느 호스트의 어느 버전이 취약한지는 대시보드가 아니라 터미널 도구(`tools/sbom_match.py`)로 확인합니다.
...
| 단계 | 조건 | 왜 이 등급인가 | 처리 |
| :--- | :--- | :--- | :--- |
| **Critical** | CISA KEV 등재 · Metasploit 모듈 공개 · SSVC 실제 악용 중 · EPSS 급증 · CVSS 9+ | 이 신호들은 "공격당할 수 있다"가 아니라 **"이미 공격에 쓰이고 있다"**는 관측 결과입니다. 정기 패치 주기를 기다리면 그 기간이 그대로 노출 구간이 되므로, 긴급 패치로 당겨야 합니다 | GitHub Issue 리포트 + Slack 즉시 알림 |
| **High** | CVSS 7~8.9 · 악용 신호는 아직 없음 | 점수는 높지만 하루 수백 건이라 전부 즉시 대응할 수는 없습니다. 다만 **신호가 하나만 붙어도 Critical**이 됩니다. 안전해서 미루는 게 아니라 순서를 두는 것입니다 | 대시보드 추적 · 매 실행마다 신호를 다시 보고 하나라도 붙으면 Critical로 자동 상향 |
...
**이 도구가 대조하는 대상은 Argus가 추적 중인 CVE입니다.** OSV 전체가 아닙니다. Argus는
최근 90일 창의 고위험 건만 추적하므로, SBOM에 있어도 그 목록에 없는 패키지는 결과에
나오지 않습니다. 그래서 실행 끝에 **확인하지 못한 패키지를 따로 밝힙니다.**

```
SBOM 1,432개 중 1,340개는 추적 목록에 없어 대조하지 못했습니다.
  openvswitch-switch, openvswitch-common, tzdata, ... 외 1,337개
  Argus는 최근 90일 창의 고위험 CVE만 추적합니다. 목록에 없다는 것은
  '그 기간에 해당 패키지의 고위험 CVE가 없었다'는 뜻이지, 취약점이 없다는 보증이 아닙니다.
```

아무 말 없이 빠지면 "안 나왔으니 안전하다"로 읽힙니다. 모를 때는 모른다고 말하는 것이
이 도구의 원칙이라, 확인 범위 밖이었다는 사실도 결과에 남깁니다.

<details>
<summary><b>패키지 이름이 SBOM에 있는데 결과에 없을 때</b></summary>

| 어떤 경우 | 왜 |
| :--- | :--- |
| 추적 목록에 그 패키지 CVE가 없음 | 최근 90일 안에 해당 패키지의 고위험 CVE가 공개되지 않았습니다. 위 안내에 이름이 찍힙니다 |
| 커널 하위 모듈 이름으로 찾는 경우 | `net: openvswitch: ...` 처럼 제목에 패키지 이름이 보여도 **리눅스 커널 CVE**입니다. 벤더·제품이 `Linux / Linux`이고 패키지는 `linux`라 **커널 묶음 줄에 포함**돼 있습니다 |
| 커널 줄이 한 줄로만 보임 | 커널은 CVE마다 고치는 게 아니라 패키지를 한 번 올리는 단위라 접어서 보여줍니다. `--expand`로 전부 펼칩니다 |
| `수정판 추정`이라 안 보임 | 기본 출력은 취약·미확인만 냅니다. `--all`로 전부 봅니다 |

`버전 미확인`은 **추적 목록에 없다는 뜻이 아닙니다.** CVE는 잡혔는데 올릴 목표 버전을
확정할 수 없다는 뜻이고, 네 가지 경우가 있습니다.

1. OSV가 그 패키지의 수정 버전을 아직 모릅니다
2. 내 배포판·릴리스가 인덱스에 없습니다 (Debian 12를 쓰는데 Debian 11 수정본만 있는 경우)
3. 설치 버전이 수정본을 넘어섰지만 릴리스가 정확히 맞지 않습니다
4. 버전 문자열을 해석하지 못했습니다

2·3번은 일부러 그렇게 둡니다. 남의 배포판 숫자로 `수정판`이라 답하면 취약한 호스트를
안전하다고 보고하게 되기 때문입니다.
...
확인 한 번으로 끝나지만, 반대로 틀리면 취약한 서버를 안전하다고 보고하게 됩니다.
...
| 용도 | 모델 | 비고 |
|---|---|---|
| **심층 분석** | `gemini-3.5-flash-lite` → `gemini-3.1-flash-lite` | JSON 모드로 구조화 출력 보장. 분석 대상이 Critical뿐이라 하루 6~30건. 하루 요청 한도 500회에 견주면 여유가 큽니다 |
| **한국어 번역** | `gemma-4-31b-it` → `gemma-4-26b-a4b-it` | CVE 제목·설명 요약. 분석과 다른 모델이라 예산을 나눠 쓰지 않습니다 |

두 단계가 모두 막히면 **미리 정해둔 문구**로 내려갑니다. 분석란은 안내문, 번역은 영문 원문으로
리포트가 나가고, 번역은 다음 실행이 소급해 채웁니다. 공급자가 통째로 멈춰도 파이프라인은
...
| **판정 경계** | CVSS·EPSS 임계값, 알림 전이 조건, 저장된 값이 `null`인 경우. 등급 판정과 Slack 즉시 알림이 같은 기준을 쓰는지 신호 조합을 전부 대조 |
| **빠뜨리지 않는 수집** | 처리 지점 전진·보류 해제·남은 몫을 다음 실행으로 넘기는 계산을 시나리오별로. 실패한 건이 처리 지점을 넘어가지 않는지 |
...
| **Metasploit** metadata | 공격 도구 등록 여부 | BSD-3-Clause (출처: Rapid7 표기) |
...
- 처음 한 번은 **Argus Maintenance → `package-index`** 를 수동 실행해 CVE↔패키지
  대응표를 만듭니다(주 1회 자동 갱신). 이게 있어야 패치 목표 버전과 SBOM 대조가 동작합니다.
