...
## 규정은 "즉시 보정"을 요구하지만, 무엇이 긴급한지는 정해주지 않습니다

전자금융감독규정 제15조 제1항 제2호는 "시스템프로그램 등의 **긴급하고 중요한 보정(patch)사항에 대하여
즉시 보정작업 실시**"를 요구합니다. ISMS-P는 `2.10.8 패치관리`와 `2.11.2 취약점 점검 및 조치`로
같은 것을 묻고, PCI DSS는 요구사항 `6.3.1`에서 취약점에 **위험 순위를 매기라고** 합니다.

공통점이 있습니다. **무엇이 긴급하고 중요한지는 규정이 정해주지 않습니다.** 그 판정을 무엇으로 하느냐가
실무이고, 판정이 없으면 "CVSS 7 이상 전부"처럼 감당 못 할 기준을 세웠다가 결국 아무것도 못 하게 됩니다.

Argus는 그 판정 근거를 자동으로 만들고, 만든 근거를 그대로 남깁니다.

| 규정이 요구하는 것 | Argus가 내놓는 것 |
| :--- | :--- |
| **자산 식별** — 무엇을 쓰고 있는지 | `syft` SBOM에서 뽑은 실제 설치 목록(`assets.json`)과의 1차 대조 |
| **위험 순위 지정** — 무엇이 긴급한지 | CISA KEV 등재 · CISA SSVC 악용 상태 · EPSS · Metasploit·ExploitDB 무기화를 종합한 3단 티어링 |
| **즉시 보정** — 어디까지 올려야 하는지 | OSV.dev 역인덱스가 배포판·릴리스별 **패치 목표 버전**을 리포트와 대시보드에 함께 제시 |
| **조치 이력·증적** — 언제 알았고 무엇을 했는지 | 발행 시각과 판단 근거가 GitHub Issue에 그대로 남아 사후 점검·감사 때 참조 |
| **지속적 재평가** — 어제 안전했던 게 오늘도 안전한지 | 저위험 CVE를 30일간 매시간 재평가하고, KEV 등재·EPSS 급등이 잡히면 그 자리에서 승격·재알림 |

마지막 항목이 사람이 하기 가장 어려운 일입니다. 한 번 "해당 없음"으로 판단하고 지나간 CVE를 매일 다시 열어보는
조직은 없습니다. 위 카드사 사례에서 문제가 된 취약점도 **공개 당시에는 이미 판단이 끝난 건**이었습니다.

한 가지 선을 그어두면, 제로데이는 이 파이프라인의 범위가 아닙니다. 패치도 정보도 없는 시점의 공격은
행위 기반 탐지 장비가 맡을 영역이고, Argus는 **공개된 다음부터 조치 전까지의 구간**을 맡습니다.
...
| 🗂️ | **조치 근거 보존** | 발행 시각 · 판단에 쓴 신호 · 데이터 출처가 GitHub Issue에 그대로 남습니다 — 사후 점검·감사 때 "언제 알았고 무엇을 근거로 판단했는지"가 증적이 됩니다 |
...
운영되게 만들었고, 사내 서버나 유료 플랜에 올리면 처리량 상한만 올라갑니다 — 로직은 그대로입니다.
