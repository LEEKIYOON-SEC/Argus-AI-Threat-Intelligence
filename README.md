...
저는 개발자가 아닙니다. 그래서 Claude 바이브 코딩으로 만들었습니다. 이 도구에 정말 필요했던 건
"무엇을 어떤 순서로 걸러야 하는지"였고 그건 실무를 하는 제가 가장 잘 아는 부분이었습니다. 문법과
라이브러리를 익히는 대신 그 기준을 설명하고 나온 결과를 검증하는 데 시간을 썼습니다. 직접 배워서
짰다면 시작도 못 했을 규모인데 실제로는 몇 주 만에 운영에 올렸고, 위험도 기준이나 리포트 구성을
바꾸고 싶을 때도 그날 안에 반영됩니다. 반복 업무를 없애려고 만든 도구인데, 만드는 과정 자체도
같은 방식으로 줄인 셈입니다.
...
추측으로 자산을 적으면 대조가 의미 없어집니다. 실제로 설치된 것만 올리려고 `syft`로 SBOM을 떴습니다.
Linux, Windows 둘 다 같은 도구를 씁니다.
...
- 경로 앞에 `dir:` 같은 스킴은 붙이지 않아도 됩니다. 지금은 호환용으로만 남아 있어서, 굳이 명시하려면
  `--from dir`을 쓰는 쪽이 권장됩니다.
- `>` 리다이렉션 대신 `-o <형식>=<파일>`을 씁니다. 형식을 여러 개 뽑을 때 한 번에 되고, Windows
  PowerShell 5.1은 `>`가 파일을 UTF-16LE로 써버려서 JSON 파서가 깨집니다. `-o`는 syft가 직접 UTF-8로
  씁니다.
- Linux에서 `/`를 훑을 땐 sudo가 필요합니다. 권한이 없으면 syft가 못 읽은 경로를 건너뛰면서
  `unable to access path=...` 경고만 남기고 정상 종료합니다. 실패로 보이지 않는데 패키지는 빠져 있어서,
  그 상태로 자산 목록을 만들면 대조가 계속 헛돕니다.

SBOM에서 벤더(`supplier`)와 제품명(`name`)을 뽑아 `assets.json`에 등록하면, 이후 수집되는 모든 CVE가
이 목록과 대조됩니다.

정작 실무에서 걸린 건 표기 불일치였습니다. SBOM은 `Microsoft Corporation`, CVE 레코드는 `microsoft`,
제품명은 `exchange_server`와 `Exchange Server`가 섞여 나옵니다. 그래서 대소문자와 `_`·공백 차이는 코드에서
흡수하고, 벤더 표기를 믿기 어려울 때는 `*`로 제품만 지정할 수 있게 했습니다.
...
**▶︎ [라이브 대시보드에서 직접 확인](https://leekiyoon-sec.github.io/Argus-AI-Threat-Intelligence/cve.html)** — 지금 운영 중인 데이터입니다.
