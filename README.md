...
파이썬 전문 개발자가 아니어도 *"무엇이 필요한지"* 를 정의하고 AI와 협업해, 실제 동작하는 위협
인텔리전스 자동화를 완성했습니다 — 도구가 아니라 **문제 정의와 보안 설계**에 집중한 결과물입니다.

취약점 관리의 출발점인 **"무엇을 지킬 것인가(자산 식별)"** 부터 → 수집·분류·분석·전달까지, 실무
프로세스를 그대로 자동화하는 것을 목표로 했습니다.
...
| | 기능 | 설명 |
|:--:|---|---|
| 🎯 | **자산 기준 위험도 티어링** | `assets.json`에 등록한 자산은 저위험까지 추적, 미등록(전체 감시)은 **고위험만** 수신해 알림 노이즈 차단 |
...
> AI 분석 결과 및 위험도 분류는 참고용이며 정확성을 보증하지 않습니다.
...
`active_rules`에 벤더/제품을 추가합니다. 무엇이 깔려 있는지 모호하면 `syft` 같은 SBOM 도구로 인벤토리를
먼저 파악한 뒤 등록하면 좋습니다.
```json
{
  "active_rules": [
...
| 표기 | 의미 |
|---|---|
| `vendor` / `product` | 특정 벤더·제품 |
| `*` / `product` | 벤더 무관, **제품만** 감시 |
| `vendor` / `*` | **특정 벤더**의 모든 제품 |
| `*` / `*` | 전체 감시 (고위험만 수신) |
