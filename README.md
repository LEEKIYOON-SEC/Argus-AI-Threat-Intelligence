...
그래서 이 순서를 그대로 자동화했습니다. Claude 바이브 코딩을 이용해 빠르게 완성했습니다.
...
한 가지 선을 그어두면, 제로데이는 이 파이프라인의 범위가 아닙니다. 패치도 정보도 없는 시점의 공격은
행위 기반 탐지 장비가 맡을 영역이고, Argus는 공개된 다음부터 조치 전까지의 구간을 맡습니다.
...
```
추적 CVE 12,005건 · 패키지 인덱스 6,328건 · 커널 별칭 168종
...
[묶음] linux-image-6.1.0-13-amd64 6.1.55-1 → 6.1.180-1 — 관련 CVE 3,885건(취약 추정 3,777 · KEV 4건)
```
...
| **놓침 방지** | 바쁘면 그냥 지나감 | 실행이 끊겨도 다음 회차가 중단된 지점부터 다시 수집 |
...
Linux 커널 CVE는 전부 `Linux / Linux`로만 표기돼 제품 단위 구분이 없고 목록의 41%를 차지합니다.
커널 필터로 **포함 · 제외 · 커널만** 셋 중에서 고르고, 각 항목이 전체 건수를 함께 보여줍니다
(`커널 제외 (4,879건 숨김)`). 나머지 제품을 훑을 때는 제외하고, 커널 패치 계획을 세울 때는
커널만 봅니다.
...
→ 권고 대응 방안 → 공개 탐지 룰 → 참고 자료 → 데이터 출처 고지 순으로 구성됩니다.</sub>

이슈는 읽는 리포트입니다. 판단에 필요한 근거만 담고, 대응 관리는 이미 쓰던 도구에서 합니다.
근거가 없는 항목은 아예 싣지 않습니다 — 공개 탐지 룰이 없으면 그 절을 통째로 빼고, OSV가
패치 버전을 모르면 그 표를 빼는 식입니다. 추적 CVE의 98.9%가 공개 룰이 없어서, "없습니다"를
설명하기 시작하면 본문 대부분이 그 얘기가 됩니다.
...
| 무료 한도 | 어떻게 지키나 |
| :--- | :--- |
| **Supabase** egress 5GB/월 | 매시간 전량이 아니라 바뀐 행만 읽어 병합합니다. 보존 정책이 120일 지난 대용량 JSONB를 비우고 180일 지난 행을 지웁니다 |
| **GitHub Actions** 2,000분/월 | 실행당 소프트 데드라인 38분. 초과 전에 스스로 마무리하고 워터마크를 저장해, 다음 실행이 그 지점부터 이어받습니다 |
| **GitHub Pages** | 데이터 파일을 커밋하지 않고 아티팩트로 배포합니다 — 저장소가 커지지 않습니다 |
| **Google AI Studio** | 역할별 RPM·TPM·RPD를 실행 간에 이어붙여 추적합니다. 한도에 닿으면 다음 모델, 그것도 안 되면 정형 폴백으로 내려갑니다 |
