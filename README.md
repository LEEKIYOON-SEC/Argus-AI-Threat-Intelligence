...
자산을 등록하면 하루치가 이렇게 줄어듭니다. 아래는 **2026년 7월 23일 실제 수집 데이터**입니다.

```
2026-07-23 수집        1,316건   ← 그날 대시보드에 쌓인 CVE
      │  ① 자산 대조 — Oracle WebLogic을 자산으로 등록했다면
      ▼
자산에 영향                22건   ← 담당자가 볼 대상, 나머지 1,294건은 여기서 종료
      │  ② 심각도 · 악용 신호
      ▼
Critical · 즉시 대응       13건   → Issue 리포트 발행 (나머지 9건은 High로 추적)
```

<sub>라이브 대시보드에서 기간 `2026-07-23` + 제품 `Oracle WebLogic Server`로 필터하면 그대로 나오는 숫자입니다.
자산 목록을 공개 저장소에 올릴 수 없어 실제 인스턴스는 전체 감시(`*/*`)로 돌리고 있고, 위 22건은 WebLogic을
자산으로 등록했을 때 걸리는 건수를 같은 데이터로 세어 본 것입니다. 최근 30일 하루 수집량은 61~1,316건입니다.</sub>

위험도는 3단계로 나눕니다. CVSS 7점대가 하루 수백 건씩 나오니 점수만으로는 걸러지지 않습니다.

| 단계 | 조건 | 왜 이 등급인가 | 처리 |
| :--- | :--- | :--- | :--- |
| **Critical** | KEV 등재 · Metasploit 무기화 · SSVC 악용 진행형 · EPSS 급증 · CVSS 9+ | 점수가 아니라 "지금 실제로 공격에 쓰이고 있다"는 관측입니다. 정기 패치 일정과 무관하게 당겨야 합니다 | GitHub Issue 리포트 + Slack 즉시 알림 |
| **High** | CVSS 7~8.9, 다른 악용 신호는 아직 없음 | 위험하지 않아서가 아니라 **아직** 신호가 없을 뿐입니다. PoC 하나만 공개돼도 Critical로 올라갑니다 | 대시보드 추적 · 신호가 생기면 자동 재알림 |
| **Low** | 그 외 | 전부 추적하면 노이즈에 묻혀 아무도 보지 않습니다 | 등록 자산이면 추적 · 아니면 최소 기록 |
...
| 신호 | 무엇인가 |
| :--- | :--- |
| **CISA KEV** | 실제 악용이 확인된 취약점만 모은 미국 정부 목록. 등재는 공격이 관측됐다는 뜻입니다 |
| **EPSS** (FIRST.org) | 향후 30일 내 악용 시도 확률 |
| **Metasploit** | 공개 침투 도구에 모듈이 올라온 상태. 기술 없이도 공격이 가능해집니다 |
| **CISA SSVC** | 악용 상태를 none / poc / active로 구분한 공식 판단 |
